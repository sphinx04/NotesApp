...
open class Theme {
    ...
    open var h1CodeFont : RPFont!
    open var h2CodeFont : RPFont!
    open var h3CodeFont : RPFont!
    open var h4CodeFont : RPFont!
    open var scCodeFont : RPFont!
    open var linkCodeFont : RPFont!
    ...
    init(themeString: String)
    {
        ...
        if let bkgColorHex = bkgColorHex
        {
            ...
            {
                themeBackgroundColor = RPColor(white: 1, alpha: 0)
    ...
    {
        codeFont = font
        var sourceCodeFont = RPFont(name: "Courier", size: font.pointSize)!
        ...
        let scCodeDescriptor = UIFontDescriptor(fontAttributes: [UIFontDescriptor.AttributeName.family:sourceCodeFont.familyName,
                                                                  UIFontDescriptor.AttributeName.face:"Bold"])
        let underlineCodeDescriptor = UIFontDescriptor(fontAttributes: [UIFontDescriptor.AttributeName.family:font.familyName,
                                                                  UIFontDescriptor.AttributeName.face:"Regular"])
        underlineCodeDescriptor.withSymbolicTraits(.traitItalic)
        #else
        ...
        h1CodeFont = RPFont(descriptor: boldDescriptor, size: font.pointSize    * 1.6)
        h2CodeFont = RPFont(descriptor: boldDescriptor, size: font.pointSize    * 1.4)
        h3CodeFont = RPFont(descriptor: boldDescriptor, size: font.pointSize    * 1.2)
        h4CodeFont = RPFont(descriptor: boldDescriptor, size: font.pointSize)
        scCodeFont = RPFont(descriptor: scCodeDescriptor, size: font.pointSize)
        linkCodeFont = RPFont(descriptor: underlineCodeDescriptor, size: font.pointSize)
